...
int collapse(HE_edge* edge) {
	int n = 0;
	if (edge->pair == nullptr) { dismiss(edge); return n; }
	edge->vert->Q = edge->Q;
	...
	if (!edge->face->flag) ++n;
	if (!edge->pair->face->flag) ++n;
	edge->face->flag = true;
	edge->pair->face->flag = true;
	dismiss(edge);
