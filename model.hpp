...
public:
    ...
    Model(string const& path, bool gamma = false) : gammaCorrection(gamma) { loadModel(path); toHalfEdge(); toGLData(); getAABB(); }

    // draws the model, and thus all its meshes
    void Draw(Shader& shader) { for (auto& mesh : meshes) mesh.Draw(shader); }

    Model() {}
    int mSize() const { return meshes.size(); }
    ...
    void getAABB() {
        ...
        zmin = meshes.front().zmin; zmax = meshes.front().zmax;
        for (auto& mesh : meshes) {
            xmin = std::min(xmin, mesh.xmin); xmax = std::max(xmax, mesh.xmax);
            ymin = std::min(ymin, mesh.ymin); ymax = std::max(ymax, mesh.ymax);
            zmin = std::min(zmin, mesh.zmin); zmax = std::max(zmax, mesh.zmax);
        }
    }
    void toHalfEdge() { for (auto& mesh : meshes) mesh.toHalfEdge(); }
